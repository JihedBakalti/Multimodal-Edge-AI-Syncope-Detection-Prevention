"""
Custom openWakeWord training script (replacement for upstream `openwakeword/openwakeword/train.py`).

Maintained for the InterSense Elysa wake-word notebook. Derived from the official openWakeWord
custom-model Colab workflow (French Piper paths, optional TFLite export).
...
# --- PyTorch classifier on top of openWakeWord embedding features (see YAML + train_model below) ---
...
def generate_and_resample_samples(**kwargs):
    # Génère les échantillons avec Piper
    generate_samples.generate_samples_onnx(**kwargs)

    # Charge et resample tous les fichiers générés
