#!/usr/bin/env python3
"""
main2.py — Continuous SpO2 monitoring → Synthetic PPG → PPGPT Anomaly Detection

Like main.py but polls spo2.json (updated by rewrite.py), re-computes new scores on changes.
Runs continuously until Ctrl+C.

Usage: python project/main2.py
"""
...
def load_spo2(json_path: str) -> tuple[float, float, float]:
    path = Path(json_path)
    if not path.exists():
        return 0.0, 0.0, 0.0  # Signal missing file
...
def save_score(timestamp, score, output_path="scores.json"):
    import json
    from pathlib import Path
...
# ──────────────────────────────────────────────────────────────────────────────
# 8. MAIN LOOP WITH POLLING
# ──────────────────────────────────────────────────────────────────────────────

def parse_args():
    p = argparse.ArgumentParser(description="Continuous SpO2 anomaly monitoring")
    p.add_argument("--input", "-i", default="spo2.json", help="JSON path")
    p.add_argument("--model", "-m", default="../HeartGPT/Model_files/PPGPT_500k_iters.pth", help="Model path")
    ...
    p.add_argument("--plot-out", default="ppgpt_anomaly_result_spo2.png", help="Plot path")
    p.add_argument("--fs", type=float, default=100.0, help="Sampling freq")
    return p.parse_args()


def run_pipeline(model: PPGPT, bpm: float, spo2: float, args, counter: int):
    print(f"\n--- Update #{counter} ---")
    ...
    # Save minimal results to results.json (overwrite each time)
    import time  # Ensure time imported
    results_data = {
        "timestamp": time.time(),
        "score": round(mae_score, 4)
    }
    with open("results.json", "w") as f:
        json.dump(results_data, f)
    print(f"  Saved results.json: timestamp={results_data['timestamp']:.1f}, score={results_data['score']}")
...
def main():
    ...
    print(f"\nMonitoring {args.input} every {args.poll_interval}s (Ctrl+C to stop)")
    print("=" * 60)
    ...
    try:
        while True:
            ...
            if (mtime > last_mtime or ts > last_ts) and bpm > 0:
                run_pipeline(model, bpm, spo2, args, counter)
                counter += 1
                last_mtime = mtime
                last_ts = ts
